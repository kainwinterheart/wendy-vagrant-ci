...
my $__functional_regexp = qr/^(PROC|LANGUAGE|TEMPLATE|TEMPLATEC|INCLUDE|TEMPLATEFF|URL|URLFF|LOAD|CTYPE|TTL|CODE|HEADER):([\w\/\-:\.\;\=,\s\+\?\&]+)/;
...
sub template_process
{
	...
	if( open( my $tfh, "<", $tplfile ) ) # or die "cant open template file " . $tplfile . $!;
	{
		@OUTPUT = <$tfh>;
		close $tfh;
	} else
	{
		assert( 0, "cant open template file " . $tplfile . $! );
	}
		...
		if( $tline =~ $__functional_regexp )
		{
				 ...
				 ( $keyword eq 'TEMPLATEC' ) )
			{
				...
				#unless( &in( $argument, @templates_used ) )
				{
					...
					my $original_hpath_value_we_should_preserve = $WOBJ -> { 'HPATH' };
					$WOBJ -> { "HPATH" } = $argument;
					...
					$WOBJ -> { 'HPATH' } = $original_hpath_value_we_should_preserve;
				}
...
sub get_all_replaces
{
	return %REPLACES;
}

sub set_all_replaces
{
	%REPLACES = @_;
}


sub localize_replaces_and_exec
{
	my $code = shift;

	assert( ref( $code ) eq 'CODE' );

	my %old = &get_all_replaces();

	my $rv = $code -> ();

	&set_all_replaces( %old );
