...
define :cpan_module, :force => nil, :version => "" do
  execute "install-#{params[:name]}" do
    if params[:force]
      command "#{node['perl']['cpanm']['path']} --force #{params[:name]}#{params[:version]}"
    else
      command "#{node['perl']['cpanm']['path']} #{params[:name]}#{params[:version]}"
    end
